package net.web.fabric.website;
    ...
    public static void start(int port, int homepage) {
        Javalin app = Javalin.create(config -> {
            config.addStaticFiles(staticFiles -> {
                ...
                staticFiles.directory = "/html";              // the directory where your files are located
                staticFiles.location = Location.CLASSPATH;
            });
            config.addStaticFiles(staticFiles -> {
                staticFiles.hostedPath = "/";                   // change to host files on a subpath, like '/assets'
                staticFiles.directory = "/html/index";              // the directory where your files are located
                staticFiles.location = Location.CLASSPATH;
            });
            config.addStaticFiles(staticFiles -> {
                staticFiles.hostedPath = "/";                                               // change to host files on a subpath, like '/assets'
                staticFiles.directory = "config/html";              // the directory where your files are located
