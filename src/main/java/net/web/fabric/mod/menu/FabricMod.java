...
import net.web.fabric.mod.menu.OptionalUtil;
import net.web.fabric.mod.menu.Mod;
...
public class FabricMod implements Mod {
    ...
    public @NotNull NativeImageBackedTexture getIcon(FabricIconHandler iconHandler, int i) {
        ...
        if ("minecraft".equals(getId())) {
            iconSourceId = modid;
            iconPath = "assets/" + modid + "/minecraft_icon.png";
        } else if ("java".equals(getId())) {
            iconSourceId = modid;
            iconPath = "assets/" + modid + "/java_icon.png";
        }
        ...
        if (icon == null) {
            ...
            return iconHandler.createIcon(FabricLoader.getInstance().getModContainer(modid).orElseThrow(() -> new RuntimeException("Cannot get ModContainer for Fabric mod with id " + modid)), "assets/" + modid + "/unknown_icon.png");
        }
        return icon;
    }
